...
private:
  void scanDirectory() {
    ...
    mCurrentFile.getParentDirectory().findChildFiles(mFiles, juce::File::findFiles, false,
                                                     "*." + mExtension);

    std::sort(mFiles.begin(), mFiles.end(), [](const juce::File &a, const juce::File &b) {
